...
The control structure of a IPN is also called {\em underlined} PN and it is the only part that can be analyzed as seen in the lectures. An IPN whose underlined net is live \emph{can} has a deadlock (e.g., the extra-condition coming from a sensor is false). An IPN whose underlined net is unbounded (respectively, not live) can be bounded (respectively, live), according with the extra-conditions. But it's better to deal with underlined PN having the good properties.
% un rdp non-borné et non-vivant peut être borné et vivant avec les donnees, voir Essai4???

The steps for validating an IPN are the following:
\begin{description}
...
Note that in the simulator as well in the embedded control, if an information concerning the data structure (e.g. a sensor value) changes but  the corresponding transition is not enabled by the marking, nothing will occur. 
...
The interpreted Petri net model (a .rdp file) must be uploaded on the NXT to be played by the token player as well  as the list of way points used for the robot navigation as described in the following.

\newpage
\section{Control of a Mindstorm Lego}
\subsection{The Mindstorm Lego}

The robot Navi used in this lab class is the one represented in fig.~\ref{robot}.a, with: a digger, 2 tractor wheels and a freewheel;  a  light sensor and  a compass sensor; 3 actuators (2 for the tractor wheels and one for control the digger). Navi has a brick Lego NXT as described in section~\ref{brick} and was conceived for the purposes of terrestrial navigation. It can follow some way points (sets of coordinates identifying a point in a plan, in our case) along a path; a section is defined as a path between two consecutive way points, from a way point W$_i$ to the next way point W$_{i+1}$.  Two particular points are defined: B (for base) and L (for delivering).

\subsection{Specification}
Navi is on an initial point with coordinates (0,0) and must a) go  first to the \emph{base}  or B point, b) follow the predefined way points until the last way point (the last one in the .wp file), then c) come back to the base and stop. 
The .wp file has the form represented in fig.~\ref{robot}.b and must also be uploaded in the NXT brick.

In its way from B point to the final way point, if Navi detects a ball (the light sensor value is more than 50) in a section [W$_i$, W$_{i+1}$],  it lowers its digger for taking the ball (without stop) and when it arrives at point W$_{i+1}$, it brings  the ball to the L \emph{delivering} point. When arriving at the L point, it raises the digger to leave the ball and comes back to W$_{i+1}$, the end way point of the section it found the ball.
 
If Navi goes through a section  without find any ball, it goes on  to the next way point in the list and so on.   When arriving at the last way point of the list (it has completed the navigation through all way points), it comes back to the base (initial WP) and stop.

Before moving through the way points, the (complex) predefined action {\tt initNave} must be executed by the token player, in order to : a) calibrate the compass, b) initialize the navigation (all coordinates are set to zero), c) read the .wp file containing the way points (the WP are not absolute but relative to (0,0)) as well B and L points. 
...
\begin{enumerate}
\item to follow all the way points contained in the .wp file (B, L and list of WP), see fig.~\ref{robot}.b;
...
\begin{center}
	...
	%	\end{verbatim}
		\end{minipage}}
	\caption{The robot for terrestrial navigation through waypoints}
\end{center}
\label{robot}
\end{figure}
...
\vspace{-.5cm}
\begin{table}[htdp]
\caption{The .net and .rdp files for the example on fig.~\ref{tokenP}.a}
\begin{center}
	\begin{tabular}{|c|c|}
	\hline
...
The NXT has an interface made up of 4 buttons: right and left arrow (gray), escape (dark gray) and enter or OK (orange), as depicted in fig.~\ref{navi}.a. When the robot is On, the screen of fig.~\ref{navi}.b  appears.
...
{\bf tina} %\footnote{Tina is a toolbox developed in the OLC group of LAAS/CNRS.} 
 is a toolbox for the edition (graphical or textual editor {\bf nd} (NetDraw)), the analysis of Petri nets (from the reachable marking graph and  the structural analysis).
{\bf tina} do the analysis of temporal PN by constructing the graph of classes.
It allows using mouse with 1, 2 or 3 buttons. Remember {\bf tina} can only edit and analyze the underlined (Interpreted) Petri net (without conditions and actions).
...
\item on the window, click Help, Setup and indicate the number of mouse buttons,
...
a marking $M$ and an arc $(M,M')$ labelled by $t$ indicates that $M \stackrel{t}{\longrightarrow} M'$. Tina has several outputs, we will use two of them: 
  ...
  \begin{enumerate}
\item  the reachability graph in a graphical mode (.aut file) whose notes ($i=1..k$) are the markings (states) and an arc is labelled by a  transition. Use menu
...
\caption{IPN Simulator Interface}
\label{simu}
\end{figure}
